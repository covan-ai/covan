...
export const EXCLUDED: Record<string, string> = {
  ...
  tool_connection_grants:
    "a permission, and the same answer connection_grants gets above, with one reason it does not have. A grant here says an agent may run a named operation at a connected application without asking — and the connection it hangs off is deliberately not in this archive, because the OAuth grant behind it lives at Composio and belongs to the install that made it. So a restored row is at best inert and at worst a standing always-allow arriving in an install where nobody was asked for it, attached by primary key to a connection somebody else made. The agents come back in full; the permissions are granted again by the people who hold them, which is the only way a permission should ever arrive anywhere. Losing them on a restore is the safe direction, because a missing grant means ask.",
  tool_connections:
