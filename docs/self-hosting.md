...
| `COMPOSIO_API_KEY`                                                | _empty_                         | Optional. Lets agents call about 1500 connected applications — Gmail, HubSpot, Linear — through Composio, which holds the OAuth grants. Blank means the feature is not offered and the Integrations page names this variable. Set it on the cron Worker too, or a scheduled run and a chat turn disagree about what exists. Composio bills per tool call; see [Integrations](integrations.md). |
