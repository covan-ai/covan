...
| `WORKER_PLAN`                                                      | _empty — means `free`_          | Optional, and **only** meaningful on Cloudflare Workers. Set it to `paid` if your Cloudflare account is on Workers Paid, and agents may spend more on one answer: 24 tool calls instead of 8, plus up to two extra rounds when a turn is still working at the end of that. Leave it unset on Workers Free — those numbers need more than the 50 subrequests a Free invocation gets, and going over is reported by the model's SDK as a connection error rather than as a limit. Unset is safe everywhere, including Node and Docker, which have no subrequest cap at all and where the smaller numbers cost nothing but a longer answer now and then. Set it on the cron Worker too. |
