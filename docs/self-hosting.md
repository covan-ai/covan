...
| `COVAN_VERSION`                                                    | `latest`                        | Which published image tag to run. `latest` follows releases; `edge` follows `main`; a semver like `0.2.0` pins one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`                        | _empty_                         | Optional. Lets a bundle sync from a Google Drive folder. Read the scope warning in [Integrations](integrations.md) first — `drive.readonly` is a restricted scope.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `SLACK_CLIENT_ID` / `SLACK_CLIENT_SECRET` / `SLACK_SIGNING_SECRET` | _empty_                         | Optional. Lets people ask an agent from Slack. All three or none: a client pair without the signing secret installs and then rejects every event.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `COMPOSIO_API_KEY`                                                 | _empty_                         | Optional. Lets agents search about 1500 applications and call the ones somebody has connected — Gmail, HubSpot, Linear — through Composio, which holds the OAuth grants. Searching is catalogue-wide; connecting is not, and about a tenth connect with a sign-in alone. Blank means the feature is not offered and the Integrations page names this variable. Set it on the cron Worker too, or a scheduled run and a chat turn disagree about what exists. Composio bills per tool call; see [Integrations](integrations.md).                                                                                                                                                                                                                                                                                                     |
| `COMPOSIO_BASE_URL`                                                | _empty — means Composio_        | Optional. Points the catalogue and the execute call at a proxy or a self-hosted Composio instead of `backend.composio.dev`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `WORKER_PLAN`                                                      | _empty — means `free`_          | Optional, and **only** meaningful on Cloudflare Workers. Set it to `paid` if your Cloudflare account is on Workers Paid, and agents may spend more on one answer: 24 tool calls instead of 8, plus up to two extra rounds when a turn is still working at the end of that. Leave it unset on Workers Free — those numbers need more than the 50 subrequests a Free invocation gets, and going over is reported by the model's SDK as a connection error rather than as a limit. Unset is safe everywhere, including Node and Docker, which have no subrequest cap at all and where the smaller numbers cost nothing but a longer answer now and then. It changes **conversations only** — a scheduled run keeps its eight either way, because a tick runs a whole batch of them inside one invocation. See [Routines](routines.md). |
...
can read on somebody else's behalf: `0041` gives `feedback` a select policy of
...
**If your workspaces use connected services, check the plan.** A routine whose
agent has tools runs the agent loop rather than one model call, and a Workers
**Free** invocation gets 50 subrequests — the batch size is sized for four
ordinary routines at twelve each, and one agent turn spends more than twelve
